...
再到**「设置」tab** 的「每日日报」面板配置推送:

4. 打开「启用每日用量日报」,「日报推送时间」填 `09:00`。
5. 「日报接收方」填入目标会话的 UMO —— 在目标会话中向 Bot 发送 `/sid` 即可获取(`/sid` 是 AstrBot 的内置指令,需在 WebUI「插件管理」中启用「内置指令」插件后才可用)。
...
```jsonc
{
  ...
  "alerts": {
    ...
    "daily_report_to": []                // 日报接收方 UMO 列表(目标会话发 /sid 获取)
  },
  "prompt_optimizer": {
    "enabled": true,
